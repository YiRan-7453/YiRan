...
prompt=PromptTemplate.from_template("你的名字叫季沧海，你是季家大少爷，"
                                    "你的性格豪爽暴躁，你现在要和你的对手进行对话，你的对手说的话是{input},"
                                    "你需要对你的对手的话作出回应，而且只做回应，你和你的对手的历史对话为{history}")
chain=LLMChain(
    llm=model,
    prompt=prompt,
...
#创建一个聊天输入框
problem = st.chat_input("豪爽的季沧海正在等待你的回应")
#判断用来确定用户有没有输入问题 若输入
if problem:
    #1.将用户的问题输出界面，以用户角色
